...
const { PrismaClient } = require('./prisma/generated/app');
  ...
  return rateLimit({
    ...
    handler: (req, res) => {
      if (req.rateLimit.count > limitBeforePenalty) {
        res.set('Retry-After', penaltyDuration);
        res.status(429).json({
...
nextApp.prepare().then(() => {
  ...
  expressApp.post('/signup', createRateLimiter(), async (req, res) => {
  ...
  expressApp.post('/login', createRateLimiter(), async (req, res) => {
    console.log('@login');
  ...
  expressApp.get('/protected', verifySession, (req, res) => {
    console.log('Custom API route hit with body:', req.body); // Log the request body for debugging
  ...
  expressApp.get('/getApp', async (req, res) => {
    try {
      const dao = await appClient.dAOs.findUnique({
        where: {
          id: 1,
        },
      });

      const dharmaBeing = await appClient.dharmaBeings.findUnique({
        where: {
          id: 1,
        },
      });
  ...
  expressApp.post('/updateApp', async (req, res) => {
    try {
      ...
      if (dao) {
        await appClient.dAOs.update({
          where: { id: 1 },
          data: { ...dao },
        });
      }
      if (dharmabeing) {
        await appClient.dharmaBeings.update({
          where: { id: 1 },
          data: { ...dharmabeing },
        });
      }
  ...
  expressApp.all('*', energyMiddleware, (req, res) => {
...
async function verifySession(req, res, next) {
  ...
  try {
    ...
    req.user = session.user; // Attach user information to the request object
    next();
  } catch (error) {
